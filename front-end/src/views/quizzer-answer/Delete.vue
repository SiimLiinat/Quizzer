<template>
    <div class="card card-body mt-5">
        <div v-if="!loading" class="card card-body">
            <h4>Quizzer's answer</h4>
            <hr />
            <dl class="row">
                <dt class = "col-sm-2">
                    Quizzer
                </dt>
                <dd class = "col-sm-10">
                    {{quizzerAnswer.quizzerId}}
                </dd>
                <dt class = "col-sm-2">
                    Answer
                </dt>
                <dd class = "col-sm-10">
                    {{quizzerAnswer.answerId}}
                </dd>
            </dl>
            <div>
                <input @click="deleteQuizzerAnswer" type="submit" value="Delete" class="btn btn-danger" />
            </div>
        </div>
        <div v-if="!loading">
            <router-link :to="'/quizzer-answer/details/' + quizzerAnswer.id">Details</router-link>
            <span> | </span>
            <router-link to="/quizzer-answers">Back to List</router-link>
        </div>
        <div v-else>
            <div class="spinner-border text-primary" role="status">
                <span class="sr-only">Loading...</span>
            </div>
        </div>
    </div>
</template>

<script lang="ts">
import { Options, Vue } from 'vue-class-component';
import store from '@/store';
import { BaseService } from '@/services/base-service';
import IQuiz from '@/domain/IQuiz'
import IQuizzerAnswer from '@/domain/IQuizzerAnswer'
@Options({
    components: {},
    props: {
        id: String,
    }
})
export default class QuizzerAnswerDelete extends Vue {
    id!: string;
    private quizzerAnswer!: IQuizzerAnswer;
    private loading: boolean = true;

    get role(): string | undefined {
        return store.state.role;
    }

    get token(): string | undefined {
        return store.state.token;
    }

    async mounted(): Promise<void> {
        if (this.role !== 'Admin') await this.$router.push('/');
        const service = new BaseService<IQuizzerAnswer>('v1/quizzerAnswers', this.token || undefined);
        await service.get(this.id).then((data) => {
            if (data.ok) {
                this.quizzerAnswer = data.data!;
            }
        });
        this.loading = false;
    }

    async deleteQuizzerAnswer(): Promise<void> {
        const service = new BaseService<IQuiz>('v1/quizzerAnswers', this.token || undefined);
        await service.delete(this.id).then((data) => {
            if (data.ok) {
                this.$router.push('/quizzer-answers')
            } else {
                console.log(data)
            }
        });
    }
}
</script>

<style scoped>

</style>
